mtNewNick Q quote services. "Quote Services"
...
bind [dict get $::mts sock] msg q request mtQS:request
bind [dict get $::mts sock] msg q delete mtQS:delete
...
proc mtQS:request {f to t} {
	...
	if {!$cando} {mtNotice Q $f "No.";return}
	mtJoin Q $chan ao
	mtPrivmsg Q $chan [format "\[\002Quotes\002\] I was requested to join this channel by %s" $f]
...
proc mtQS:delete {f to t} {
	...
	if {!$cando} {mtNotice Q $f "No.";return}
	mtPrivmsg Q $chan [format "\[\002Quotes\002\] I was requested to part this channel by %s" $f]
	mtPart Q $chan "Radar contact lost."
...
proc quoteservdo {from chan msg} {
	...
	switch -nocase -glob -- $subcmd {
		"sea*" {
			...
			if {[llength $qts]} {mtPrivmsg Q $chan "\[\002Quotes\002\] Found quotes numbered #[join $qts ",#"]"} {
				mtPrivmsg Q $chan "\[\002Quotes\002\] No quotes found for pattern"
			}
		}
		"vi*1st*ma*" {
			set ptn "*[join $para " "]*"
			set qts [quotesearch $chan $ptn]
			if {[llength $qts]} {set qtn [lindex $qts 0];mtPrivmsg Q $chan "\[\002Quotes\002\] Quote number #$qtn:";mtPrivmsg Q $chan "\[\002Quotes\002\] [nda get "quoteserv/quotes/$ndacname/$qtn"]"} {
				mtPrivmsg Q $chan "\[\002Quotes\002\] No quotes found for pattern"
			}
		}
		"ad*" {
			...
			mtPrivmsg Q $chan "\[\002Quotes\002\] Added quote number #$qtn to database."
		}
		"de*" {
			set qtn "[lindex $para 0]"
			if {![string is integer $qtn]} {mtPrivmsg Q $chan "\[\002Quotes\002\] Please use a valid integer (without the #)"}
			if {150>[nda get "regchan/$ndacname/levels/[string tolower [tnda get "login/$::netname([dict get $::mts sock])/$from"]]"]} {mtPrivmsg Q $chan "\[\002Quotes\002\] Check your privilege."}
			nda set "quoteserv/quotes/$ndacname/$qtn" ""
			mtPrivmsg Q $chan "\[\002Quotes\002\] Blanked quote number #$qtn in database."
		}
		"vi*" {
			set qtn "[lindex $para 0]"
			if {![string is integer $qtn]} {mtPrivmsg Q $chan "\[\002Quotes\002\] Please use a valid integer (without the #)"}
			...
			if {$qt != ""} {
				mtPrivmsg Q $chan "\[\002Quotes\002\] Quote number #$qtn:"
				mtPrivmsg Q $chan "\[\002Quotes\002\] $qt"
			}
		}
		"he*" {
